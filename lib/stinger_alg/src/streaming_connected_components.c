...
int scc_main_demo(struct stinger * S, int64_t nv, int64_t *batch,int64_t batch_size){
	...
	while(1){
		...
		stinger_scc_insertion(alg->stinger,alg->stinger->max_nv,scc_internal,&stats,alg->insertions,batchSizeExample);
		stinger_scc_deletion(alg->stinger,alg->stinger->max_nv,scc_internal,&stats,alg->deletions,batchSizeExample);
