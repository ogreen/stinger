...
#define PARTITIONS_DEFAULT  -1
#define NUM_BATCHES_DEFAULT -1
#define SIZE_BATCH_DEFAULT  -1

static long batch_size = SIZE_BATCH_DEFAULT;
static long nbatch = NUM_BATCHES_DEFAULT;
...
void prettyPrint(iterationInfo* extraInfo, int64_t nbatch, int64_t nv, int64_t ne, int64_t npartitions, int64_t firstIteration){
...
//  parse_args (argc, argv, &initial_graph_name, &action_stream_name, &batch_size, &nbatch);
  ...
  if(batch_size==SIZE_BATCH_DEFAULT)
    batch_size=naction/10;

  if(nbatch==NUM_BATCHES_DEFAULT)
    nbatch=(naction/batch_size)+((naction%batch_size)>1);

  int64_t partitionArray[]={2,4,8,16,32,64};
  int64_t partitionArrayLen=sizeof(partitionArray)/sizeof(int64_t);

  if(npartitions!=PARTITIONS_DEFAULT){
    partitionArray[0]=npartitions;
    partitionArrayLen=1;
  }
...
//    BATCH_SIZE_CHECK();
  ...
  for (int64_t part=0; part<partitionArrayLen; part++){
    ...
    prettyPrint(extraInfo,nbatch,nv,currentNe,npartitions, part==0);
